...
def test_xpath_20_function_string_join_matches(client, live_server):
    ...
    res = client.post(
        url_for("edit_page", uuid="first"),
        data={"include_filters": "string-join(//*[contains(@class, 'sametext')]|//*[matches(@class, 'changetext')], ', ')",
              "url": test_url,
              "tags": "",
              "headers": "",
              'fetch_backend': "html_requests"},
    ...
    assert b"Some text thats the same, Some new text" in res.data #in selector
