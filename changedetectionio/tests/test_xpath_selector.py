...
# XPATH1 tests
# Handle utf-8 charset replies https://github.com/dgtlmoon/changedetection.io/pull/613
def test_check_xpath1_filter_utf8(client, live_server):
...
# Handle utf-8 charset replies https://github.com/dgtlmoon/changedetection.io/pull/613
def test_check_xpath1_text_function_utf8(client, live_server):
...
def test_check_markup_xpath1_filter_restriction(client, live_server):
...
# actually only really used by the distll.io importer, but could be handy too
def test_xpath1_check_with_prefix_include_filters(client, live_server):
    res = client.get(url_for("form_delete", uuid="all"), follow_redirects=True)
    assert b'Deleted' in res.data
...
def test_xpath1_various_rules(client, live_server):
    # Just check these don't error
    # live_server_setup(live_server)
    with open("test-datastore/endpoint-content.txt", "w") as f:
        f.write("""<html>
